...
# from volttron.platform.auth import (AuthEntry, AuthFile, AuthFileIndexError, AuthFileEntryAlreadyExists,
#                                     AuthFileUserIdAlreadyExists, AuthEntryInvalid)
# from volttron.platform import jsonapi

# @pytest.fixture(scope='function')
# def auth_file_platform_tuple(make_volttron_home_func_scope):
...
# @pytest.mark.auth
# def test_auth_file_api(auth_file_platform_tuple, auth_entry1, auth_entry2, auth_entry3):
#     auth_file = auth_file_platform_tuple
...
# @pytest.mark.auth
# def test_remove_auth_by_credentials(auth_file_platform_tuple, auth_entry1, auth_entry2, auth_entry3):
#     auth_file = auth_file_platform_tuple
...
#     auth_file = auth_file_platform_tuple
#     cred = 'C' * 43
#     auth_file.add(AuthEntry(credentials=cred, groups=['group_1'], roles=['role_b']))
#     auth_file.load()
...
#     roles = {'role_a': ['cap_a_1', 'cap_a_2'], 'role_b': ['cap_b_1'], 'role_c': ['cap_c_1'], 'role_d': ['cap_d_1']}
#     groups = {'group_1': ['role_a', 'role_c'], 'group_2': ['role_b']}
...
#     assert len(results) == 1
#     entry = results[0]
#     assert set(entry.capabilities) == set(['cap_a_1', 'cap_a_2', 'cap_b_1', 'cap_c_1'])
...
#     expected["capabilities"] = {'can_publish_temperature': None, 'edit_config_store': {'identity': entries[0].user_id}}
...
#             assert entry.capabilities == {'can_publish_temperature': None, 'edit_config_store': {'identity': 'user1'}}
#         elif entry.user_id == "user2":
#             assert entry.capabilities == {'blah': None, 'foo': None, 'edit_config_store': {'identity': 'user2'}}
