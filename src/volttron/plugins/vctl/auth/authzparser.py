...
class AuthzCtlParser(ControlParser):
    ...
    def configure(self, ctx):
        ...
        # This maintains compatibility while leveraging the new plugin architecture
        add_authz_parser(
            lambda name, **kwargs: ctx.register_subcommand(
                kwargs.pop('subparser', ctx._subparsers),
                name,
                **kwargs
            ) if 'subparser' in kwargs else ctx.register_command(name, **kwargs),
            ctx.filterable
        )
