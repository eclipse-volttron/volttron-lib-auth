...
class VolttronAuthService(AuthService, Agent):
    ...
    def _send_update(self):
        ...
        _log.debug(f"-------------------------------- after getting peerlist to send auth updates, peers are: {peers}")
    ...
    #             user = self.zap_authenticate(domain, address, kind, credentials)
    ...
    # def zap_authenticate(self, domain: str, address: str, mechanism: str, credentials: str):
    ...
    def _update_topic_permission_tokens(self, identity, not_allowed):
        ...
        """
        read_tokens = [
            "{instance}.{identity}".format(instance=self.core._instance_name, identity=identity),
            "__pubsub__.*",
        ]
        write_tokens = ["{instance}.*".format(instance=self.core._instance_name, identity=identity)]

        if not not_allowed:
            write_tokens.append("__pubsub__.{instance}.*".format(instance=self.core._instance_name))
        else:
