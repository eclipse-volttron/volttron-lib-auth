...
#from volttron.decorators import authenticator, authorizer, authservice, service
...
@service
class AuthenticationService(AuthService, Agent):
    ...
    def _send_auth_update_to_pubsub(self):
        user_to_caps = self.get_user_to_capabilities()
        _log.debug(f"CORE IS: {self.core}")
